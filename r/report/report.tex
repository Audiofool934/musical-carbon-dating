...
\subsection{OLS Regression Results (Baseline)}
Table \ref{tab:mlr_coeffs} displays the coefficients from the initial Ordinary Least Squares (MLR) model, prior to WLS refinement.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Predictor} & \textbf{Est.} & \textbf{SE} & \textbf{t} & \textbf{p} \\
        \midrule
        Intercept & 2001.14 & 0.03 & 72034.08 & $<0.001$ \\
        Loudness & 6.56 & 0.05 & 141.78 & $<0.001$ \\
        Tempo & 0.70 & 0.03 & 24.19 & $<0.001$ \\
        Duration & -0.76 & 0.03 & -26.02 & $<0.001$ \\
        Mode & -0.86 & 0.03 & -30.53 & $<0.001$ \\
        Time Sig & -0.07 & 0.03 & -2.26 & $0.024$ \\
        Acousticness & 0.45 & 0.04 & 11.84 & $<0.001$ \\
        Danceability & 4.11 & 0.04 & 116.35 & $<0.001$ \\
        Energy & -0.88 & 0.05 & -16.29 & $<0.001$ \\
        Instrumentalness & 0.51 & 0.03 & 16.34 & $<0.001$ \\
        Liveness & -0.32 & 0.03 & -10.96 & $<0.001$ \\
        Speechiness & -0.25 & 0.03 & -8.16 & $<0.001$ \\
        Valence & -4.48 & 0.04 & -125.34 & $<0.001$ \\
        \bottomrule
    \end{tabular}
    \caption{Baseline OLS (MLR) Coefficients. Comparing these with WLS results (Table \ref{tab:coeffs}) reveals how weighting corrects for heteroscedasticity.}
    \label{tab:mlr_coeffs}
\end{table}

\subsection{Multicollinearity Diagnostics}
Variance Inflation Factors (VIF) were computed to assess multicollinearity (Table \ref{tab:vif}). All values are well below the threshold of 5 (or 10), indicating no severe multicollinearity.
...
\section{Analysis Pipeline Walkthrough}
The R analysis script (\texttt{analysis.R}) implements a comprehensive six-stage pipeline relative to the project methodology.

\subsection{Data Loading \& Preprocessing}
The script begins by loading the raw dataset (\texttt{tracks.csv}) and applying critical filters:
\begin{itemize}
    \item \textbf{Temporal Filtering}: Restricting data to the 1960--2020 range to ensure cultural relevance.
    \item \textbf{Popularity Threshold}: Excluding tracks with popularity $<30$ to remove outliers and noise.
    \item \textbf{Feature Selection}: Retaining 12 core audio features. Notably, \textbf{Key} was excluded based on preliminary testing ($p=0.94$).
    \item \textbf{Standardization}: Applying Z-score normalization ($x' = \frac{x - \mu}{\sigma}$) using parameters derived solely from the \textbf{training set} (80\%) to prevent data leakage into the test set (20\%).
\end{itemize}

\subsection{Exploratory Data Analysis (EDA)}
Before modeling, the script generates a \textbf{Correlation Heatmap} to identify collinearity (e.g., Energy vs. Loudness) and histograms to assess feature distributions (often multimodal).

\subsection{Regression Modeling Phases}
The core analysis proceeds in iterative phases:
\begin{enumerate}
    \item \textbf{Phase I (SLR)}: A simple linear regression on \textbf{Loudness} establishes the baseline "Loudness War" trend ($R^2 \approx 0.14$).
    \item \textbf{Phase II (MLR)}: A full multiple linear regression provides a baseline $R^2 \approx 0.25$.
    \item \textbf{Phase III (Diagnostics)}:
    \begin{itemize}
        \item \textbf{Partial F-Test}: Confirmed significant non-linearity ($p < 0.001$), primarily in quadratic terms.
        \item \textbf{Breusch-Pagan Test}: Identified severe heteroscedasticity ($\chi^2 \approx 20,754$), necessitating Robust Regression.
    \end{itemize}
    \item \textbf{Phase IV (WLS)}: The final model employs \textbf{Weighted Least Squares}, where weights are inversely proportional to the variance of residuals ($w_i = 1 / \sigma_i^2$), stabilizing the variance (Table \ref{tab:coeffs}).
\end{enumerate}

\subsection{Model Selection \& Validation}
To verify the feature set, the script runs:
\begin{itemize}
    \item \textbf{Stepwise Regression}: Optimizing for AIC.
    \item \textbf{LASSO (L1 Regularization)}: Using Cross-Validation to find $\lambda_{min}$.
\end{itemize}
The comparison (Figure \ref{fig:model_selection}) showed marginal gains, confirming the robustness of the MLR/WLS feature set.

\subsection{Final Evaluation}
The script concludes by applying the WLS model to the \textbf{Test Set}:
\begin{itemize}
    \item Computing \textbf{RMSE} (Root Mean Squared Error) and \textbf{MAE} (Mean Absolute Error).
    \item Calculating the \textbf{Nostalgia Index} ($\Delta = |Year_{pred} - Year_{actual}|$).
    \item Generating the final \textbf{Predicted vs. Actual} hexbin plot and \textbf{Error by Era} boxplot.
\end{itemize}
