...
\begin{frame}{Phase I: The "Loudness War" (SLR)}
    ...
    \begin{columns}
        ...
        \textbf{Results}:
        \begin{itemize}
            \item $t$-stat: \textbf{183.7} (Highly Significant).
            \item $\beta_{loud} \approx 1.2$ years/dB.
            \item $R^2 = 0.144$.
        \end{itemize}
...
\begin{frame}{Phase II: Multiple Linear Regression (Baseline)}
    ...
    \begin{itemize}
        ...
        \item \textbf{Result ($R^2$)}: \textbf{0.296}
        \item \textbf{RMSE}: 12.06 years.
    \end{itemize}
...
\begin{frame}{Phase III: Diagnostic Audit (1/2)}
    \textbf{Test 1: Linearity}
    \begin{itemize}
        \item \textbf{Method}: Partial F-Test adding quadratic terms ($X^2$).
        \item \textbf{Result}: $F \approx 304.7$, $p \approx 0.00$.
        \item \textbf{Finding}: \textcolor{red}{Assumption Violated.} Relationship is non-linear.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Result}: Max VIF (Energy) = \textbf{3.78}. All VIFs $< 5.0$.
...
\begin{frame}{Phase III: Diagnostic Audit (2/2)}
    ...
    \begin{columns}
        \column{0.5\textwidth}
        \includegraphics[width=\linewidth]{residuals_mlr_residuals.png}
        
        \column{0.5\textwidth}
        \begin{itemize}
            \item \textbf{Visual}: Residuals fan out and have sharp boundaries.
            \item \textbf{Test}: Breusch-Pagan.
            \item \textbf{Statistic}: $\chi^2 = \mathbf{22,043}$.
            \item \textbf{p-value}: $< 0.001$.
            \item \textbf{Finding}: \textcolor{red}{CRITICAL FAILURE.} Variance is strictly time-dependent.
        \end{itemize}
    \end{columns}
    
    \vspace{0.3cm}
    \textbf{Implication}: OLS estimators are unbiased but inefficient. Standard errors are wrong. \textbf{We must use WLS.}
\end{frame}
...
\begin{frame}{Phase V: Weighted Least Squares (WLS)}
    ...
    \begin{columns}
        ...
        \textbf{Metrics:}
        \begin{itemize}
            \item Weighted $R^2$: \textbf{0.77} (Trend Fit)
            \item Unweighted Test $R^2$: $\approx 0.30$ (Raw Data)
            \item Test \textbf{RMSE}: 12.0 years
            \item Test \textbf{MAE}: 9.3 years
        \end{itemize}
    \end{columns}
...
\begin{frame}{Phase VI-A: Technological Drivers ("The Sound of Efficiency")}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item Mean \texttt{Loudness} has skyrocketed over time.
            \item Yet, \texttt{Energy}'s coefficient is \textbf{negative} ($\beta \approx -5.4$) when controlling for Loudness.
...
\begin{frame}{Phase VI-B: Cultural Evolution ("The Mood of an Era")}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textbf{Danceability} ($\beta \approx +24.0$): The single strongest predictor. Rhythm is the defining feature of modernity.
            \item \textbf{Valence} ($\beta \approx -16.5$): Optimism has collapsed.
        ...
        \begin{itemize}
            ...
            \item WLS Coefficient: \textbf{Positive} ($\beta \approx +2.08$).
...
\begin{frame}{The Nostalgia Index}
    ...
    \begin{table}[]
        ...
        \begin{tabular}{ll ccc}
            ...
            \textit{Physical} (Dua Lipa) & 2020 & 2009.0 & \textbf{11.0} & \textbf{Retro Aesthetic} \\
            \textit{Blinding Lights} (Weeknd) & 2019 & 2004.2 & \textbf{14.8} & \textbf{80s Revival} \\
            \bottomrule
        \end{tabular}
    \end{table}
\end{frame}
