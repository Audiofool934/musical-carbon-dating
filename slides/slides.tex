...
\begin{frame}{Phase V: Weighted Least Squares (WLS)}
    ...
    \begin{columns}
        ...
        \textbf{Metrics:}
        \begin{itemize}
            \item Weighted $R^2$: \textbf{0.275} (Valid Trend Fit)
            \item F-statistic: $\mathbf{4.94 \times 10^3}$ (Highly Significant)
            \item Unweighted Test $R^2$: $\approx 0.24$ (Baseline MLR)
            \item Test \textbf{RMSE}: 12.27 years
            \item Test \textbf{MAE}: 9.30 years
        \end{itemize}
    \end{columns}
    
    \vspace{0.2cm}
    \footnotesize{\textit{Note: The $R^2$ of 0.275 reflects the high variance of musical evolution. The model captures the trend, but stylistic diversity remains high.}}
\end{frame}
